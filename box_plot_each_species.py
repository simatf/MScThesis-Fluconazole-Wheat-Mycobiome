...
for species in long_df["Putative species"].unique():
    ...
    if (flu > 0).any():  
        ...
        plt.text(0.5, y_max*1.01, f"p = {p:.3f}", ha="center")
        ...
        plt.text(1, y_mid, "NA", ha="center", va="center", color="black", fontsize=12)
# for species in long_df["Putative species"].unique():
#     sub = long_df[long_df["Putative species"] == species]
...
#     plt.text(0.5, 0.8, f"p = {p:.3f}", ha="center")
    ...
    plt.xticks(ticks= [0, 1], labels= ['Control', 'Fluconazole'], fontsize= 10)
    # plt.yticks(ticks= plt.yticks()[0], fontsize= 12)
    plt.tight_layout()
    plt.gcf().set_size_inches(5, 12)
    plt.savefig(f"NEW_box_each_species/NEW/00{species}.png", dpi=300)
    #plt.show()
